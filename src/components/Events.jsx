...
const Events = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch user data
        const userResponse = await axios.get("https://club-manager-3k6y.vercel.app/api/auth/user", {
        ...
        // Fetch all events
        const eventsResponse = await axios.get("https://club-manager-3k6y.vercel.app/api/events", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const completeRegistration = async (eventId, paymentDetails = null) => {
    ...
    try {
      ...
      const requestUrl = `https://club-manager-3k6y.vercel.app/api/events/${eventId}/register`;
