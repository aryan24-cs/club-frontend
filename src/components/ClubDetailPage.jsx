...
const ClubDetailPage = () => {
  ...
  const fetchClubData = useCallback(async () => {
    try {
      ...
      const clubResponse = await axios.get(
        `https://club-manager-3k6y.vercel.app/api/clubs/${clubId}`,
        {
      ...
      const eventsResponse = await axios.get(
        `https://club-manager-3k6y.vercel.app/api/events?club=${clubData._id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      const membersResponse = await axios.get(
        `https://club-manager-3k6y.vercel.app/api/clubs/${clubData._id}/members`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleLeaveClub = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `https://club-manager-3k6y.vercel.app/api/clubs/${club._id}/leave`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleContactSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://club-manager-3k6y.vercel.app/api/clubs/${clubId}/contact`,
        {
  ...
  const handleWhatsAppLinkSubmit = async (whatsappLink) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
        `https://club-manager-3k6y.vercel.app/api/clubs/${club._id}`,
        { whatsappLink },
        { headers: { Authorization: `Bearer ${token}` } }
      );
              ...
              <motion.button
                ...
                onClick={() => {
                  if (
                    ...
                    axios
                      .delete(`https://club-manager-3k6y.vercel.app/api/clubs/${club._id}`, {
                        headers: {
